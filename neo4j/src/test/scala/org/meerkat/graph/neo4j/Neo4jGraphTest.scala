...
abstract class Neo4jGraphTest(name: String) extends FunSuite {
  ...
  def createParser: AbstractCPSParsers.AbstractSymbol[Entity, Entity, _, _]

  def doTest(parser: AbstractCPSParsers.AbstractSymbol[Entity, Entity, _, _], graph: Neo4jInput, db: GraphDatabaseService)
