...
package object converters {
  private def constructNodeFromDFSSequence(
      current: SPPFNode,
      parent: SPPFNode,
      sequence: Iterator[SPPFNode]): SPPFNode = {
    ...
    clone match {
      case packed: PackedNode =>
        if (packed.leftChild != null) {
          packed.leftChild =
            constructNodeFromDFSSequence(packed.leftChild, packed, sequence)
              .asInstanceOf[NonPackedNode]
        }
        if (packed.rightChild != null) {
          packed.rightChild =
            constructNodeFromDFSSequence(packed.rightChild, packed, sequence)
              .asInstanceOf[NonPackedNode]
        }
      case nonpacked: NonPackedNode =>
        if (nonpacked.first != null) {
          val next =
            if (nonpacked.isAmbiguous) sequence.next else nonpacked.first
          nonpacked.first =
            constructNodeFromDFSSequence(next, nonpacked, sequence)
              .asInstanceOf[PackedNode]
          nonpacked.rest = null;
        }
    }
  ...
  def constructTreeFromDFSChoices(
      root: NonPackedNode,
      sequence: Iterator[SPPFNode]): NonPackedNode = {
    if (sequence.isEmpty)
      return root

    val start = root.copy()

    constructNodeFromDFSSequence(start, null, sequence)
      .asInstanceOf[NonPackedNode]
  }

  private def disambiguateNode(node: SPPFNode,
                               sequence: Iterator[SPPFNode]): Seq[SPPFNode] = {
    node match {
      case nonpacked: NonPackedNode =>
        if (nonpacked.first != null) {
          val next =
      ...
      case _ =>
        ...
        if (packed.rightChild != null)
          packed.rightChild =
            packed.rightChild.copy().asInstanceOf[NonPackedNode]

        packed.children
    }
  }

  def constructTreeFromBFSChoices(
      root: NonPackedNode,
      sequence: Iterator[SPPFNode]): NonPackedNode = {
    if (sequence.isEmpty)
      return root;
    ...
    while (currentLevel.nonEmpty) {
      currentLevel =
        currentLevel.flatMap(node => disambiguateNode(node, sequence))
    }
  ...
  private def tryToCountTrees(node: SPPFNode,
                              visited: mutable.Set[SPPFNode]): Int = {
    ...
    val count = node match {
      case packed: PackedNode =>
        packed.children
          .map(tryToCountTrees(_, visited))
          .sum - (packed.children.size - 1)
      case vertex: VertexNode[_] => 1
      case terminal: EdgeNode[_] => 1
      case epsilon: EpsilonNode  => 1
      case nonpacked: NonPackedNode =>
        nonpacked.children.map(tryToCountTrees(_, visited)).sum
    }
  ...
  def extractNonAmbiguousSPPFs(root: NonPackedNode,
                               converter: Converter): Stream[NonPackedNode] =
  ...
  private def extractPath(root: tree.Tree, isMostLeft: Boolean): Seq[Int] =
    root match {
      case node @ tree.RuleNode(_, children) =>
        extractPath(children.head, isMostLeft) ++
  ...
  def extractPath(root: tree.Tree): Seq[Int] = root match {
    case node @ tree.RuleNode(_, children) =>
      extractPath(children.head, true) ++
        children.tail.flatMap(child => extractPath(child, false))
