...
object GraphInputTestCases extends Matchers {
  def nonAmbiguousGrammarTwoPathsTestCorrectness(converter: Converter): Unit = {
    ...
    val rule1 = Rule(
      S.symbol,
      Sequence(EdgeSymbol("x"), EdgeSymbol("+"), EdgeSymbol("x")))
    val rule2 = Rule(
      S.symbol,
      Sequence(EdgeSymbol("x"), EdgeSymbol("-"), EdgeSymbol("x")))
  ...
  def nonAmbiguousGrammarInfiniteNumberOfPathsTestCorrectness(
      converter: Converter): Unit = {
    var S: Nonterminal[String, Nothing] = null
    S = syn("a" ~ S ~ "b" | "a" ~ "b")

    val graph = Graph((0 ~+#> 1)("a"),
                      (1 ~+#> 2)("a"),
                      (2 ~+#> 0)("a"),
                      (0 ~+#> 3)("b"),
                      (3 ~+#> 0)("b"))
    ...
    val rule1 =
      Rule(S.symbol, Sequence(EdgeSymbol("a"), S.symbol, EdgeSymbol("b")))
    val rule2 = Rule(S.symbol, Sequence(EdgeSymbol("a"), EdgeSymbol("b")))

    val minTree = RuleNode(rule2, Seq(terminalNode("a"), terminalNode("b")))
    val expectedTrees = Stream
      .iterate(minTree)(tree =>
        RuleNode(rule1, Seq(terminalNode("a"), tree, terminalNode("b"))))
      .zipWithIndex
      .filter({ case (_, index) => index % 3 == 2 })
      .map({ case (tree, _) => tree })

    expectedTrees.zip(trees).take(10).foreach {
      case (expected, actual) =>
        compareTreesIgnoringExtents(expected, actual) shouldBe true
    }
  }
  ...
  def getTrees[V](
    ...
    extractTreesFromSPPF(getSPPFs(S, input).right.getOrElse(null)._1,
                         converter)(input)
  }
}
